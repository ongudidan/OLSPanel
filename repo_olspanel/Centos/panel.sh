...
create_database_and_user() {
    ...
    # Generate a random password for the new user or load existing
    local DB_PASSWORD=""
...
import_database() {
    ...
    # Check if database already has tables to avoid overwriting existing data
...
install_powerdns_and_mysql_backend() {
    ...
    if [[ "$OS_NAME" == "centos" || "$OS_NAME" == "almalinux" ]]; then
        if [[ "$OS_VERSION" == "7" ]]; then
            PKG_MANAGER="yum"
            sudo yum install -y epel-release
            sudo curl -o /etc/yum.repos.d/powerdns-auth-43.repo http://127.0.0.1:8000/repo-files/centos-auth-43.repo
...
echo "y" | sudo ufw enable
    ...
    sudo iptables -I INPUT 1 -p tcp --dport 40110:40210 -j ACCEPT
    sudo iptables -I OUTPUT 1 -p tcp --dport 40110:40210 -j ACCEPT
...
sudo openssl req -new -newkey rsa:2048 -days 365 -nodes -x509 \
  ...
  -out /etc/olspanel/cert.pem && \
sudo chmod 600 /etc/olspanel/key.pem && \
sudo chmod 644 /etc/olspanel/cert.pem

fi

if [ ! -f "/etc/olspanel/port" ]; then
printf "%s" "$new_port" | sudo tee /etc/olspanel/port > /dev/null
fi


printf "%s" "/usr/local/olspanel/mypanel" | sudo tee /etc/olspanel/base_dir > /dev/null
   

if [ ! -f "/etc/olspanel/env_home" ]; then
printf "%s" "/usr" | sudo tee  /etc/olspanel/env_home > /dev/null

fi


if [ ! -f "/etc/olspanel/env_pkg" ]; then
SITE_PACKAGES=$(find /root/venv/lib/ -maxdepth 1 -type d -name "python*" | head -n 1)/site-packages

printf "%s" "$SITE_PACKAGES" | sudo tee  /etc/olspanel/env_pkg > /dev/null
...
if [[ "$ARCH" == "aarch64" || "$ARCH" == "armv7l" ]]; then
    ...
    wget http://127.0.0.1:8000/extra/openssl_lib/arm/libcrypto.so.3 -O /usr/local/lib/olspanel/lib/libcrypto.so.3
    wget http://127.0.0.1:8000/extra/openssl_lib/arm/libssl.so.3 -O /usr/local/lib/olspanel//lib/libssl.so.3
	wget http://127.0.0.1:8000/extra/arm/olspanelcp -O /usr/local/bin/olspanelcp
else
    wget http://127.0.0.1:8000/extra/openssl_lib/libcrypto.so.3 -O /usr/local/lib/olspanel/libcrypto.so.3
    wget http://127.0.0.1:8000/extra/openssl_lib/libssl.so.3 -O /usr/local/lib/olspanel/libssl.so.3
	wget http://127.0.0.1:8000/extra/olspanelcp -O /usr/local/bin/olspanelcp
...
sudo tee /etc/systemd/system/cp.service > /dev/null << 'EOF'
[Unit]
Description=OLSPanel
After=network.target mysql.service  # Ensure MySQL starts before this service

[Service]
User=root
Group=www-data
ExecStart=/usr/local/bin/olspanelcp
...
sed -i '/listener panel[[:space:]]*{/,/^[[:space:]]*}/d; /listener panel IPv6[[:space:]]*{/,/^[[:space:]]*}/d' "$httpd_file"
...
copy_mysql_password() {
    ...
    local target_dir="/usr/local/olspanel/mypanel/etc/"
...
set_ownership_and_permissions() {
    ...
    sudo chown -R www-data:www-data /usr/local/olspanel/mypanel
    sudo chmod -R 755 /usr/local/olspanel/mypanel
...
add_backup_cronjobs() {
    ...
    # Define cron jobs
    local CRON_JOBS="\
0 * * * * /usr/local/bin/olspanel --backup --hour
0 0 * * * /usr/local/bin/olspanel --backup --day
0 0 * * 0 /usr/local/bin/olspanel --backup --week
0 0 1 * * /usr/local/bin/olspanel --backup --month
0 0 * * * /usr/local/bin/olspanel --check_version
0 */3 * * * /usr/local/bin/olspanel --limit_check
...
if [ -f "/usr/local/olspanel/mypanel/etc/mysqlPassword" ]; then
...
install_pip
if [[ ("$OS_NAME" == "centos" || "$OS_NAME" == "almalinux") && ("$OS_VERSION" == "7" || "$OS_VERSION" == "8") ]]; then
        /root/venv/bin/python3.12 /usr/local/olspanel/mypanel/manage.py reset_admin_password "$(get_password_from_file "/root/db_credentials_panel.txt")"
    else
        /root/venv/bin/python3 /usr/local/olspanel/mypanel/manage.py reset_admin_password "$(get_password_from_file "/root/db_credentials_panel.txt")"
    fi
...
curl -sSL http://127.0.0.1:8000/extra/dovecot/re_conf.sh | sed 's/\r$//' | bash
sleep 3
