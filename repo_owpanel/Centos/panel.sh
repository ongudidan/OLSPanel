...
php_version=$(echo "$version" | awk '{print substr($0,1,1) "." substr($0,2,1)}')
            ...
            sudo sed -i 's/^upload_max_filesize\s*=.*/upload_max_filesize = 2048M/' "$target_ini"
            sudo sed -i 's/^post_max_size\s*=.*/post_max_size = 2048M/' "$target_ini"
            sudo sed -i 's/^memory_limit\s*=.*/memory_limit = 2048M/' "$target_ini"
